<?php
    ...
    $sql = "SELECT t.id, aa.name AS outgoingAccount, ab.name AS incomingAccount, t.amount, ua.name AS createdBy, ub.name AS authorizedBy, uc.name AS madeBy FROM `Transfer` t JOIN Account aa ON t.outgoingAccount = aa.id JOIN Account ab ON t.incomingAccount = ab.id JOIN User ua ON t.createdBy = ua.id LEFT JOIN User ub ON t.authorizedBy = ub.id LEFT JOIN User uc ON t.madeBy = uc.id";

<?php
    ...
    include_once("../components/only-superuser.php");
    ...
    $sql = "SELECT madeBy FROM `Transfer` WHERE id = ?";
    ...
    if ($transfer["madeBy"] !== NULL) {
        echo json_encode(["success" => FALSE, "error" => "Transferência já marcada como feita"]);
        exit();
    }

    $sql = "UPDATE `Transfer` SET madeBy = %d, madeAt = NOW() WHERE id = ?";
